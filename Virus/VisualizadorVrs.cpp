...
VisualizadorVrs *VisualizadorVrs::ptr;
bool dibujando = false;
char grafostr[] = "No hay grafo cargado! por favor cree o cargue un grafo para visualizar";

VisualizadorVrs::VisualizadorVrs(const GrafoGnr<NdoVrt>& g) : grafo(g), simulador(&grafo) {
...
void VisualizadorVrs::visualizar() const {
...
void VisualizadorVrs::visualizar(int cItr, int ios, double vsc, double rc, double grc)
{
...
void VisualizadorVrs::simular()
{
    simulador.simular(1, info.ios, info.vsc, info.rc, info.grc);
    glutPostRedisplay();
}

double VisualizadorVrs::generaPos() {
...
void VisualizadorVrs::atragantador() {
...
void VisualizadorVrs::dibujar_circulo(double radio, double x, double y) {
    glBegin(GL_POLYGON);
...
void VisualizadorVrs::linker(int lineas, vector<int>& arrV, int vrt) {
...
void VisualizadorVrs::recurCircles() {
    int cntAdy;
    cntVrt = grafo.obtTotVrt();
    for (int i = 0; i < cntVrt; i++) {
        //int vrt = vrtPopular();
...
int VisualizadorVrs::vrtPopular() {
    int vrtPop, cont = 0;
    while (cont < cntVrt) {
...
void VisualizadorVrs::estadoVrt(int vrt) {
...
void VisualizadorVrs::keyboard(unsigned char key, int x, int y)
{
    if (ptr == NULL) return;
    if (ptr->sim)
    {
...
void VisualizadorVrs::display(void) {
...
void VisualizadorVrs::idle(void) {
    if (ptr != NULL)
    {
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        ptr->recurCircles();
