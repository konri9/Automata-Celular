...
VisualizadorAves *VisualizadorAves::ptr;
...
void VisualizadorAves::visualizar() const {
...
void VisualizadorAves::visualizar(int cItr, int ios, double vsc, double rc, double grc)
{
...
void VisualizadorAves::simular()
{
    simulador.simular(1, info.ios, info.vsc, info.rc, info.grc);
    glutPostRedisplay();
}

double VisualizadorAves::generaPos() {
...
void VisualizadorAves::atragantador() {
...
void VisualizadorAves::dibujar_circulo(double radio, double x, double y) {
    glBegin(GL_POLYGON);
...
void VisualizadorAves::linker(int lineas, vector<int>& arrV, int vrt) {
...
int VisualizadorAves::vrtPopular() {
    int vrtPop, cont = 0;
    while (cont < cntVrt) {
...
void VisualizadorAves::keyboard(unsigned char key, int x, int y)
{
    if (ptr == NULL) return;
    if (ptr->sim)
    {
...
void VisualizadorAves::idle(void) {
    if (ptr != NULL)
    {
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        ptr->recurCircles();
